...
API_CALLS_LOG = Path(os.environ.get("HH_LOG_DIR", "/app/logs")) / "api_calls.jsonl"
...
class BaseClient:
    ...
    def request(
        ...
        **kwargs: Any,
        ...
        with self.lock:
            ...
            if (
                wait := delay - time.monotonic() + self.previous_request_time
            ) > 0:
                logger.debug("wait %.2fs before request", wait)
                time.sleep(wait)
            ...
            try:
                ...
                self._log_api_call(method, full_url, response.status_code)
    ...
    @staticmethod
    def _log_api_call(method: str, url: str, status: int) -> None:
        try:
            API_CALLS_LOG.parent.mkdir(parents=True, exist_ok=True)
            entry = {
            ...
            logger.debug("Failed to write API call log")
